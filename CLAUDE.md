# CLAUDE.md

This file provides guidance to Claude Code when working with code in this repository.

## Project Overview

Embedded firmware for an STM32F103XE (ARM Cortex-M3) robotics platform. Controls motors, servos, and encoders over UART or CAN bus using FreeRTOS. Written in C++17 (application logic) and C11 (HAL/system code) with no exceptions, no RTTI, and no threadsafe statics.

## Build Commands

```bash
# Configure (choose preset: Debug, RelWithDebInfo, Release, MinSizeRel)
cmake --preset Debug

# Build
cmake --build --preset Debug

# Flash via SWD (requires STM32_Programmer_CLI)
STM32_Programmer_CLI -c port=SWD -w build/Debug/bolt.elf -hardRst
```

Toolchain: `arm-none-eabi-gcc` with Ninja generator. No unit test framework — testing is done on hardware.

## Architecture

### FreeRTOS Task Pipeline

Four tasks form a command processing pipeline communicating via FreeRTOS queues:

1. **vCommand_Task** — Receives raw bytes from UART (`USART1`) or CAN bus
2. **vProcess_Task** — Feeds bytes into `FrameParser` state machine, dispatches parsed frames via `AppVisitor`
3. **vQuery_Task** — Sends response frames back over UART or CAN
4. **vLed_Task** — LED heartbeat indicator

### Frame Protocol

Binary protocol: `[SOF=0xAA][TYPE:1][LEN:1][PAYLOAD:LEN][CRC16-CCITT:2][EOF=0x55]`

See @docs/FIRMWARE.md for full spec

### Key Design Patterns

- **Visitor pattern**: `FrameVisitor` base class → `AppVisitor` handles each frame type. Decouples frame definitions (`frames.hpp`) from processing logic.
- **Template Handle Registry**: `HandleRegistry<Controller, HandleType>` maps HAL peripheral handles to C++ controller instances, bridging C interrupt callbacks to OOP controllers.
- **Hardware abstraction interfaces** (`Bolt/Inc/bolt/interface/`): `OutputPin`, `SerialPort`, `AsyncSerialPort`, `CanBus`, `PWMTimer`, `CountTimer` — all concrete implementations wrap STM32 HAL calls.
- **Compile-time communication switch**: `USE_CANBUS` macro selects between UART and CAN bus modes.

### Code Layout

- `Bolt/` — Application code (the code you'll modify most)
  - `Inc/bolt/interface/` — Hardware abstraction interfaces and implementations
  - `Inc/bolt/controller/` — Motor, servo, encoder controllers
  - `Inc/bolt/` — Frame definitions, parser state machine, visitor, protocol constants
  - `Src/application.cpp` — FreeRTOS task definitions and main application wiring
  - `Inc/peripherals.hpp` — Global peripheral instances shared across tasks
  - `Inc/queues.hpp` / `Src/queues.cpp` — FreeRTOS queue declarations
- `Core/` — STM32CubeMX-generated HAL configuration (regenerated from `bolt.ioc`)
- `Drivers/` — STM32F1xx HAL drivers and CMSIS (vendor-provided, do not edit)
- `Middlewares/` — FreeRTOS kernel (vendor-provided, do not edit)

### Controller Details

- **MotorController**: 4 motors via PWM on TIM1/TIM8, dead-zone handling, max pulse 3600
- **ServoController (PWM)**: 4 servos via TIM1/TIM8 PWM channels
- **ServoController (UART)**: 6 servos via serial protocol on USART3, pulse range 96-4000
- **EncoderController**: 4 encoders on TIM2/3/4/5, 2464 CPR, 20ms sampling, low-pass filtered (alpha=0.2)

## Conventions

- All application code lives under `bolt::` namespace with sub-namespaces (`serial`, `can`, `timer`, `controller`, `pin`, `registry`)
- Most implementation is in `.hpp` headers (heavy use of templates and inline functions)
- Inter-task communication uses FreeRTOS queues, never shared globals
- `Core/` files are auto-generated by STM32CubeMX from `bolt.ioc` — edit the `.ioc` file instead of modifying generated code directly
